...
toOptional (IsZ listZipper) = Full listZipper
toOptional _ = Empty
...
findLeft f (ListZipper l a r) =
  ...
  in case xs of
    Nil      -> IsNotZ
    (h :. t) -> IsZ (ListZipper t h (reverse r1 ++ r2))
...
findRight f (ListZipper l a r)=
  ...
  in case xs of
    Nil      -> IsNotZ
    (h :. t) -> IsZ (ListZipper (reverse l1 ++ l2) h t)
...
moveLeftLoop (ListZipper l a r) =
  case l of
    Nil -> case reverse (a :. r) of
      Nil      -> error "Both sides can't be empty!"
...
moveLeft (ListZipper Nil _ _) =
  IsNotZ
moveLeft (ListZipper l a r) =
  let (h :. t) = l
  in IsZ (ListZipper t h (a :. r))
...
moveRight (ListZipper _ _ Nil) =
  IsNotZ
moveRight (ListZipper l a r) =
  let (h :. t) = r
  in IsZ (ListZipper (a :. l) h t)
...
swapLeft (ListZipper Nil _ _) =
  IsNotZ
swapLeft (ListZipper l a r) =
  let (h :. t) = l
  in IsZ (ListZipper (a :. t) h r)
...
swapRight (ListZipper _ _ Nil) =
  IsNotZ
swapRight (ListZipper l a r) =
  let (h :. t) = r
  in IsZ (ListZipper l h (a :. t))
...
moveLeftN n =
  if n < 0
    then (moveLeftN (n + 1) -<<) . moveRight
    else (moveLeftN (n - 1) -<<) . moveLeft
...
moveLeftN' n lz@(ListZipper l _ r) =
  let len = length (if n < 0 then r else l)
  in if abs n > len
    then Left len
    else Right (getZ (moveLeftN n lz))
...
-- prop> \lz -> rights (end lz) == Nil
  ...
  getZ (nth (length (toList lz) - 1) lz)
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft a2 (ListZipper l a r) =
  ListZipper (a :. l) a2 r
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight a2 (ListZipper l a r) =
  ListZipper l a2 (a :. r)
...
-- /Tip:/ Use `zipWith`
  (ListZipper lf f rf) <*> (ListZipper l a r) =
    ListZipper (zipWith ($) lf l) (f a) (zipWith ($) rf r)
...
instance Extend ListZipper where
  (<<=) ::
    ...
    -> ListZipper b
  (<<=) f z =
    let h = (<$>) (\z' -> (f z', z')) . toOptional
    in ListZipper (unfoldr (h . moveLeft) z) (f z) (unfoldr (h . moveRight) z)
...
instance Traversable ListZipper where
  traverse f (ListZipper l a r) =
    lift3 ListZipper (traverse f l) (f a) (traverse f r) 
  -- Solution2:  getZ . fromList <$> traverse f (toList a)
...
getZ ::
  ...
  -> ListZipper a
getZ (IsZ x) = x
getZ IsNotZ = error "Not a zipper!"
